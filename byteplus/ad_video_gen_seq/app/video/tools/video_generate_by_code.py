...
async def video_generate(
        ...
        max_wait_seconds (int):
            ...
            Optional per item:
                - first_frame (str | None):
                    Code (⌥code format) or Base64 string (data URL) for the **first frame** (role = `first_frame`).
                    Use when you want the clip to start from a specific image.

                - last_frame (str | None):
                    Code (⌥code format) or Base64 string (data URL) for the **last frame** (role = `last_frame`).
    ...
    Model text commands (append after the prompt; unsupported keys are ignored by some models):
        --rs / --resolution <value>       Video resolution. Common values: 480p, 720p, 1080p.
                                          Default depends on model (e.g., dreamina-seedance-2-5: 1080p,
        ...
        --dur / --duration <seconds>      Clip length in seconds. Seedance 2.5 supports **4–30 s**.
                                          Default varies by model; 5–10 s per shot is a sensible
                                          default, but honor explicit user requests up to 30 s.

        --fps / --framespersecond <int>   Frame rate. Common: 24 (model-dependent).
    ...
    Constraints & Tips:
        - Keep prompt concise and focused (recommended <= 500 characters); too many details may distract the model.
            ...
            params = [
                {
                    ...
                    "prompt": "A kitten rides a skateboard through the park. --rs 720p --rt 16:9 --dur 5 --fps 24 --wm false"
                },
                {
                    "video_name": "city_night.mp4",
                    "prompt": "Time-lapse style of a city under neon lights. --rs 720p --rt 16:9 --dur 5 --fps 24 --seed 7"
                },
            ]
            ...
            params = [
                {
                    ...
                    "first_frame": "⌥abc12",
                    "last_frame": "⌥xyz34",
                    "prompt": "A brand logo transforming from line art to full color. --rs 1080p --rt 1:1 --dur 6 --fps 24 --cf true"
                }
            ]
    """
    for item in params:
        if "prompt" in item:
            item["prompt"] = (
                "(Very light incidental action sounds are allowed, but no human voices, "
                "no background music, no sound effects, no narration, no voice-over.) "
                f"{item['prompt']}"
            )
