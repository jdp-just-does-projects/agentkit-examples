...
class UrlShortener:
    ...
    def url2code(self, original_url: str) -> str:
        """
        Take a URL string and return a short ID
        """
        try:
    ...
    def code2url(self, short_id: str) -> Optional[str]:
        """
        Take a short ID and return the original URL
        """
        return self._short_to_long.get(short_id, short_id)

    def replace_in_text(self, text: str) -> str:
        """
        Take a long string, extract the short IDs and seamlessly replace them with the original URLs
        """
    ...
    def extract_ids_to_urls(self, text: str) -> list[str]:
        """
        Extract all short IDs from a string and convert them to a list of URLs
        """
        # Pattern matches ⌥<code> where code is 5 characters
        pattern = r"⌥([0-9a-zA-Z]{5})"
...
def upload_file_to_tos(
    ...
    # but fall back to BYTEPLUS_* directly so this helper also works standalone.
