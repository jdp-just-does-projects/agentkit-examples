...
VEFAAS_REGION = os.getenv("APP_REGION", "ap-southeast-1")
VEFAAS_FUNC_ID = os.getenv("_FAAS_FUNC_ID", "")
agent_card_builder = AgentCardBuilder(
    agent=agent,
    provider=AgentProvider(
        organization="BytePlus Agent Development Kit (VeADK)",
        url=f"https://console.byteplus.com/vefaas/region:vefaas+{VEFAAS_REGION}/function/detail/{VEFAAS_FUNC_ID}",
    ),
)
...
async def combined_lifespan(app: FastAPI):
    # Initialize the browser used for web page parsing
    await _init_browser()
