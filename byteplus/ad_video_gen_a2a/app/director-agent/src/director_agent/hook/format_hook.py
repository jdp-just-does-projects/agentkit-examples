...
def fix_output_format(
    *,
    ...
    """
    Check whether the output format meets the requirements, and try to repair it if not.
    Possible cases:
    Case 1. (valid -> valid) No schema; return the original llm_responses directly.
    Case 2. (valid -> valid) Schema present; output needs no repair and matches the schema; return llm_responses.
    Case 3. (invalid -> invalid) Schema present; output needs no repair but does not match the schema; return the original llm_responses. Log it.
    Case 4. (invalid -> invalid) Schema present; output needs repair and the repair fails; return the original llm_responses. Log it.
    Case 5. (**invalid -> valid**) Schema present; output needs repair, the repair succeeds and the result matches the schema; return the repaired llm_responses.
    Case 6. (invalid -> invalid) Schema present; output needs repair, the repair succeeds but the result does not match the schema; return the original llm_responses. Log it.
    ...
    try:
        ...
        try:
            ...
            llm_response = llm_response_validate_error(
                llm_response,
                "DirectorAgent output does not match the expected schema and could not be repaired; please retry.",
            )
    ...
    try:
        ...
        llm_response = llm_response_validate_error(
            llm_response,
            "DirectorAgent output does not match the expected schema; please retry.",
        )
