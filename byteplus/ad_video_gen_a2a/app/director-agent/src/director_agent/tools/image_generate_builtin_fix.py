...
async def image_generate(tasks: list[dict], tool_context) -> Dict:
    ...
    if model.startswith("doubao-seedream-3-0"):
        logger.error(
            f"Image generation by Doubao Seedream 3.0 ({model}) is deprecated. Please use a newer Seedream model (e.g., dola-seedream-5-0-pro-260628) instead."
        )
        return {
            "status": "failed",
            "success_list": [],
            "error_list": [
                "Image generation by Doubao Seedream 3.0 ({model}) is deprecated. Please use a newer Seedream model (e.g., dola-seedream-5-0-pro-260628) instead."
            ],
        }
