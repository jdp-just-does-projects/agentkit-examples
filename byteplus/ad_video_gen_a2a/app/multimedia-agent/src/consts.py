...
def set_veadk_environment_variables():
    ...
    This sample deliberately does *not* pin model names or API bases here:
    each service is configured through its `config.yaml` (see
    `config.yaml.example`), and veadk lets an environment variable that is
    already set win over the matching `config.yaml` key — so setting MODEL_*
    here would silently override the user's config file. Each service ships
