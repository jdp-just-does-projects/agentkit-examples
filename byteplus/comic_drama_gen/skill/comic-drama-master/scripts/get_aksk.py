...
"""
Get BytePlus / VolcEngine AK/SK credentials for comic-drama-master skill.
...
Priority order:
  ...
  2. Generic env vars: VOLCENGINE_ACCESS_KEY / VOLCENGINE_SECRET_KEY
     (veadk mirrors BYTEPLUS_* onto these when CLOUD_PROVIDER=byteplus)
  3. BytePlus env vars: BYTEPLUS_ACCESS_KEY / BYTEPLUS_SECRET_KEY
  4. VeFaaS IAM credential (automatic when running inside VeFaaS)
...
def get_aksk() -> dict:
    ...
    # Priority 3: BytePlus environment variables (standalone script runs;
    # inside the agent server veadk has already mapped these onto VOLCENGINE_*)
    if not (ak and sk):
        ak = os.getenv("BYTEPLUS_ACCESS_KEY")
        sk = os.getenv("BYTEPLUS_SECRET_KEY")
        if ak and sk:
            logger.debug("Successfully got AK/SK from BYTEPLUS_ACCESS_KEY/SECRET_KEY.")

    # Priority 4: VeFaaS IAM (runs automatically inside VeFaaS runtime)
    if not (ak and sk):
