...
Provides functionality to upload files to BytePlus TOS object storage and returns a signed access URL
...
def upload_file_to_tos(
    ...
    Args:
        ...
        region: TOS region, defaults to ap-southeast-1
    ...
    Environment variables required:
        BYTEPLUS_ACCESS_KEY (or VOLCENGINE_ACCESS_KEY): BytePlus access key
        BYTEPLUS_SECRET_KEY (or VOLCENGINE_SECRET_KEY): BytePlus secret key
    ...
    # veadk maps BYTEPLUS_* onto VOLCENGINE_* when CLOUD_PROVIDER=byteplus,
    # but fall back to BYTEPLUS_* directly so this tool also works standalone.
    access_key = os.getenv("VOLCENGINE_ACCESS_KEY") or os.getenv("BYTEPLUS_ACCESS_KEY")
    secret_key = os.getenv("VOLCENGINE_SECRET_KEY") or os.getenv("BYTEPLUS_SECRET_KEY")
    session_token = ""
    ...
    if not access_key or not secret_key:
        logger.info(
            "Error: BYTEPLUS_ACCESS_KEY and BYTEPLUS_SECRET_KEY are not provided or IAM Role is not configured."
        )
        return None
    ...
    client = None
    try:
        # Initialize TOS client (BytePlus TOS endpoint)
...
if __name__ == "__main__":
    ...
    if os.path.exists(test_file):
        ...
        if url:
            ...
            logger.info("\nPlease check:")
            logger.info(
                "1. Whether environment variables BYTEPLUS_ACCESS_KEY and BYTEPLUS_SECRET_KEY are set"
            )
