...
> - The standalone CLI reads `VOLCENGINE_ACCESS_KEY` / `VOLCENGINE_SECRET_KEY` from the **`.env` in the directory you run it from** — run every `agentkit` command below from this directory. It does not use the `MODEL_AGENT_API_KEY` in `.env` (the deployed runtime authenticates to Ark through its IAM role).
...
This is **Code Coach**, an AI coding-exercise trainer built on Volcano Engine AgentKit **without writing any application code**. It mirrors a job-training platform loop: *assign → plan → review → test → evaluate*.
...
flowchart TB
    ...
    subgraph agentkit["Volcano Engine AgentKit (cn-beijing)"]
        direction TB
    ...
    ark["Volcano Engine Ark (方舟)<br/>auth via the runtime's IAM role"]
...
The deployed runtime calls Ark through its IAM role, so **no Ark API key is needed for the cloud deployment**. Only create one (from the Ark "API Keys" page) if you want to run the local `agentkit harness dev` server.
...
Skills live in **Skill Spaces** — versioned, shareable containers on the AgentKit skill platform. Create one in the **Skills Center** of the [Volcano Engine AgentKit Console](https://console.volcengine.com/agentkit/region:agentkit+cn-beijing/overview) (e.g. `training-skills`), or reuse an existing one. Note its id (`ss-...`); you can list them at any time (after *Configure Environment Variables* below) with:
...
- The **standalone `agentkit` CLI** reads `VOLCENGINE_ACCESS_KEY` / `VOLCENGINE_SECRET_KEY` from the `.env` in the current directory. Volcano Engine is the CLI's default provider, but every command in this README passes `--provider volcengine` explicitly so it also works in a shell where `AGENTKIT_CLOUD_PROVIDER=byteplus` is exported (e.g. left over from the BytePlus demos). An AK/SK already exported in your shell, or an `agentkit login` SSO session, wins over `.env`.
- [`scripts/publish_skill.py`](scripts/publish_skill.py) loads `.env` itself (values in `.env` win over the shell), and uses `SKILL_SPACE_ID` as the default target space. If your shell exports `CLOUD_PROVIDER=byteplus` (left over from the BytePlus demos), `unset CLOUD_PROVIDER AGENTKIT_CLOUD_PROVIDER` first — it would point veADK's TOS client at the BytePlus endpoints.
...
**Option A — script (repeatable, CI-friendly).** Automates what the console does: zip → platform TOS skill bucket (`agentkit-platform-cn-beijing-<account-id>-skill`, created on first use) → `CreateSkill` API:
...
1. Open the [Volcano Engine AgentKit Console](https://console.volcengine.com/agentkit/region:agentkit+cn-beijing/runtime) → **Runtime** → `code-coach` and click **Online test** (top right).
...
For direct HTTP integration use stable `user_id` / `session_id` headers plus the API key and endpoint from the runtime's **Quick call** section in the console (e.g. `https://xxxxx.apigateway-cn-beijing.volceapi.com`).
...
- `agentkit --provider volcengine runtime logs code-coach` streams the runtime's logs — the first place to look when a turn fails. `agentkit --provider volcengine runtime show code-coach --json` shows the released version, the bound tool id, and the environment derived from `harness.yaml`.
- **`Missing Volcengine credentials`** from any `agentkit` command: you are not running it from this directory (no `.env` found). **`Missing BytePlus credentials`**: your shell exports `AGENTKIT_CLOUD_PROVIDER=byteplus` and the command was run without `--provider volcengine`.
- **Turn 1 invents its own exercise** instead of `clean_average`: the skill is not attached or not yet `running` — check `agentkit --provider volcengine skill show s-...` and `grep skills -A1 harness.yaml`, then redeploy.
- **`run_code` fails with a missing tool id:** the sandbox is not bound. Re-run `agentkit --provider volcengine runtime update code-coach --tool-id t-... --auto-release`, or inject the variable directly: `agentkit --provider volcengine runtime update code-coach --envs-json '[{"Key":"AGENTKIT_TOOL_ID","Value":"t-..."}]' --auto-release`.
...
- **Model:** any Ark model id (or `ep-...` endpoint id) enabled on your account works: `agentkit harness set --model-name <model>` and redeploy.
