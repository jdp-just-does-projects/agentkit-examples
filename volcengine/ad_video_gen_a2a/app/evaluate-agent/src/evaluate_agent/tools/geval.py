...
DEFAULT_EVALUATE_ITEM_MODEL_NAME = "doubao-seed-2-1-turbo-260628"
...
async def repair_evaluate_input(
    ...
    for shot in media_list:
        ...
        # First build the reference parts, shared within the same shot
        reference_part_list = []
        ...
        for i, media_url in enumerate(media_url_list):
            ...
            text_part = {
                ...
                "text": (
                    ...
                    + f", and the following {len(reference_media_list)} images are all reference images."
                    if len(reference_media_list) > 0
                    else "" + "Evaluate the media items as required and output a result in the required format."
                ),
            }
...
async def evaluate_media(
    ...
    # Async handler for a single message
    async def process_message(msg):
        response = await client.responses.create(
            model=os.getenv("MODEL_EVALUATE_ITEM", DEFAULT_EVALUATE_ITEM_MODEL_NAME),
    ...
    # Process all messages concurrently with asyncio.gather
    ...
    # Post-processing: merge results by shot_id and keep media_id ordering
    merged_result = {}
    for item in result:
        shot_id = item.get("shot_id")
        # Convert media_id to int for correct sorting
        media_id = int(item.get("media_id", 0))

        if shot_id not in merged_result:
            merged_result[shot_id] = {
                "shot_id": shot_id,
                "items": [],  # Store all items first, keeping media_id for sorting
            }
        merged_result[shot_id]["items"].append(
            (media_id, item.get("scores"), item.get("reason"))
        )

    # Sort each shot_id's results by media_id ascending and build the final format
    final_result = []
    for shot_id, data in merged_result.items():
        # Sort by media_id ascending
        sorted_items = sorted(data["items"], key=lambda x: x[0])

        # Extract the scores and reason lists
    ...
    # Build the return value: construct ScoredImageList / ScoredVideoList directly and dump to dict
    # Index the original input by shot_id to fill in metadata
    ...
    # Assemble the output structure for the media type
    if media_type == "image":
        scored_image_list = []
        for shot_id, data in merged_result.items():
            shot = shot_index.get(shot_id, {})
            media_entries = shot.get("media", [])
            # Map evaluation results to {media_id: (score, reason)}
