# 绘本故事视频生成 Agent（火山引擎版）

> English documentation: [README_en.md](README_en.md)

## 概述

本样例是一个基于火山引擎 AgentKit 与 VeADK 的"绘本故事视频生成"（Fable Storybook）Agent。
...
- 将成片上传到火山引擎 TOS 对象存储，并返回可访问的签名 URL
...
在火山引擎控制台的产品搜索中搜索"方舟"（Ark），在模型开通页面确认以下模型已开通：
...
**注意**：此处假设 `DATABASE_TOS_BUCKET` 与 `MODEL_AGENT_API_KEY` 已在环境中定义。`agentkit` CLI 会从当前目录的 `.env` 读取 `VOLCENGINE_ACCESS_KEY` / `VOLCENGINE_SECRET_KEY` 用于部署认证，但不会读取其他变量；如果它们保存在 `.env` 中，请先导出到当前 shell：
...
> 目的：修改后会在镜像构建阶段预装 video-clip-mcp，确保 Runtime 启动时即可用。
