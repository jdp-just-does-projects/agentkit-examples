...
本样例是一个基于火山引擎 AgentKit 与 VeADK 的单智能体电商营销视频生成器。
...
- **图生视频**：2x2 分镜图通过 `reference_images` 参数（而非首尾帧）传给 Seedance 2.5，生成一条连续视频
...
Agent、图像与视频模型名及 API 地址默认取 [`consts.py`](consts.py) 中的值（cn-beijing 方舟 Endpoint），如需覆盖：
...
**注意**：以下命令假定 `MODEL_AGENT_API_KEY` 已在当前 shell 中导出；CLI 认证所需的 `VOLCENGINE_ACCESS_KEY` 与 `VOLCENGINE_SECRET_KEY` 可以导出到 shell，也可以直接写在运行目录的 `.env` 中（agentkit CLI 会读取，见 [`.env.example`](.env.example) 顶部说明）。
...
可以。默认生成 9:16、1080P、15 秒的视频；如明确要求横屏、方形或自定义时长，Agent 会按要求生成。Seedance 2.5 支持 4 到 30 秒的时长。
