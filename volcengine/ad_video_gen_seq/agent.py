...
# veadk's built-in fallbacks lag the current model generation (notably
# Seedance 2.0 for video and Seedream 5.0 non-pro for images), so a run
# without a `config.yaml` silently lands on older models. veadk has already
# flattened `config.yaml` / `.env` into os.environ during its import above, so
# filling in defaults with setdefault here keeps the documented precedence:
